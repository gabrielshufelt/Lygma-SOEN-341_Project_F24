...
RSpec.describe Team, type: :model do
  ...
  it 'does not allow more than 6 users to belong to the same team' do
    ...
    # Add 6 students
    6.times do
      team.add_student(User.create!(role: 'student', first_name: 'Test', last_name: 'User',
                                    email: Faker::Internet.email, password: 'password', sex: 'other'))
    end

    expect(team.students.count).to eq(6)

    # Try to add a 7th student
    result = team.add_student(User.create!(role: 'student', first_name: 'Extra', last_name: 'User',
                                           email: Faker::Internet.email, password: 'password', sex: 'other'))
